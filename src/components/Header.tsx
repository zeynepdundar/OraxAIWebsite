...
function Header({
  ...
  return (
    ...
    >
      {/* Subtle top accent */}
      <div
        className={`absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/20 to-transparent transition-opacity duration-300 ${
          overHero ? 'opacity-100' : 'opacity-60'
        }`}
      />
...
>
  ...
  />
        ...
        >
          <span
            className={`my-[0.3rem] block h-0.5 w-6 transition-colors ${
              overHero ? 'bg-white' : 'bg-white/90'
            }`}
          />
          <span
            className={`my-[0.3rem] block h-0.5 w-6 transition-colors ${
        ...
        {/* Navigation */}
        <nav
          className={`absolute left-4 right-4 top-[calc(100%+0.55rem)] rounded-[20px] border border-white/10 bg-[#08172e]/95 p-4 shadow-[0_12px_40px_rgba(0,0,0,0.25)] backdrop-blur-md md:static md:flex md:items-center md:gap-6 md:rounded-none md:border-0 md:bg-transparent md:p-0 md:shadow-none md:backdrop-blur-none ${
        ...
        >
          <ul className="flex flex-col gap-2 md:flex-row md:gap-5">
            {navItems.map(([id, label]) => (
              <li key={id} className="list-none">
                <a
                  className={`inline-flex items-center rounded-lg px-2.5 py-1.5 font-semibold transition-all duration-200 ${
                    overHero
          ...
          <div className="mt-4 flex flex-col gap-3 md:mt-0 md:flex-row md:items-center md:gap-5">
            {/* Language switch */}
            <div
              className={`flex cursor-pointer items-center gap-2 rounded-lg px-3 py-2 font-semibold transition-all duration-200 ${
                overHero
                  ? 'text-white/90 hover:bg-white/10 hover:text-white'
                  : 'text-white/80 hover:bg-white/10 hover:text-white'
              }`}
            >
              <Globe
                className={`h-4 w-4 ${
                  overHero ? 'text-white/90' : 'text-white/70'
                }`}
              />
